/******************************************************************************
// Aufgabe:    Testbench for readline()   
// File:       rltest.c
...
//*****************************************************************************
// Purpose:    use readline() to read data from pipe and compare with sent data
// Parameters: fd    -> filedescriptor of pipe
//             len   -> length of read buffer 
//             check -> if set, check if written and read data are equal
// Return:     0 if check failed, else number of characters read by readline()
...
int doReadLine(int *fd, int len) {
    ...
    rc = readline(readBuffer, len);         // read one line
...
//*****************************************************************************
// Purpose:     Main program for testing readline()
// Function:    Since readline reads from "stdin", one can only send strings
//              to readline from software by connecting "stdin" to a file.
...
//              process and compares it with the string by readline(). The 
...
int main(void) {
    ...
    if (pid == 0) {                                 // child
        ...
        rv = sendAndCheck(pid, fd, size, 0) - 1;
